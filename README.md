...
NAVER Cloud Platform 자산을 조회만 하는 **read-only MCP server**입니다.

생성·변경·삭제 없이 VPC, 서버, 로드밸런서, SourceDeploy 등의 현재 상태를 읽어오는 도구만 제공합니다. MCP client에서 다음과 같은 요청을 처리할 수 있습니다.

```text
현재 서버 목록 보여줘
VPC와 서브넷 구조를 정리해줘
로드밸런서 리스너와 타겟 그룹 연결 상태 확인해줘
ACG와 Network ACL 인바운드 규칙을 점검해줘
외부 노출 위험이 있는 자산 JSON을 점검해줘
```
...
| 카테고리 | 제공 기능 |
| --- | --- |
| Server | VPC 서버 목록/상세, 네트워크 인터페이스 목록/상세, 공인 IP 목록 조회 |
| VPC | VPC, Subnet, ACG(+rule), Network ACL(+rule), Route Table(+route/subnet), NAT Gateway, VPC Peering 조회 |
| Load Balancer | 로드밸런서 목록/상세, 리스너, 리스너 규칙 조회 |
| Target Group | 타겟 그룹 및 타겟 상태 조회 |
| SourceDeploy | 프로젝트, 스테이지, 시나리오, 배포 히스토리 조회 |
| Local Advisor | 붙여넣은 자산 JSON에서 public IP, LB, ACG, open CIDR 흔적 점검 |
...
- `ncloud_list_network_interfaces` — 네트워크 인터페이스 목록 조회 (부착된 ACG/서버 포함)
- `ncloud_get_network_interface` — 단일 네트워크 인터페이스 상세 조회
- `ncloud_list_public_ips` — 공인 IP 인스턴스 목록 조회
...
- `ncloud_list_acg_rules` — 단일 ACG의 인바운드/아웃바운드 규칙 조회
- `ncloud_list_network_acls` — VPC 내 Network ACL 목록 조회
- `ncloud_list_network_acl_rules` — 단일 Network ACL의 인바운드/아웃바운드 규칙 조회
- `ncloud_list_route_tables` — VPC 내 Route Table 목록 조회
- `ncloud_list_routes` — 단일 Route Table의 경로 목록 조회
- `ncloud_list_route_table_subnets` — 단일 Route Table에 연결된 Subnet 목록 조회
- `ncloud_list_nat_gateways` — VPC 내 NAT Gateway 목록 조회
- `ncloud_list_vpc_peerings` — VPC Peering 목록 조회
...
- `ncloud_advise_inventory_risks` — 붙여넣은 자산 JSON에서 public IP, load balancer, ACG, open CIDR 흔적 점검
...
```text
이 자산 JSON을 ncloud_advise_inventory_risks로 점검해줘
```
...
아래 도구는 TypeScript build와 read-only policy smoke test(정적 검증)는 통과했으나, 라이브 API 검증은 대기 중입니다.

- ACG rule list
- Network interface list / detail
- Public IP list
- Network ACL list / rule list
- Route Table list / route list / route table subnet list
- NAT Gateway list
- VPC Peering list
